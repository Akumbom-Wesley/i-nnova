...
class ContactFormTest extends TestCase
{
    ...
    {
        $response = $this->post('/contact', $this->validSubmission());
    ...
    {
        $this->post('/contact', $this->validSubmission(['email' => 'not-an-email']))
    ...
    {
        $this->post('/contact', $this->validSubmission(['message' => 'hi']))
    ...
    public function test_the_honeypot_refuses_an_automated_submission(): void
    {
        ...
        $this->post('/contact', $this->validSubmission(['website' => 'https://spam.example']))
    ...
    public function test_repeated_submissions_are_throttled(): void
    {
        for ($attempt = 0; $attempt < 6; $attempt++) {
            $this->post('/contact', $this->validSubmission(['email' => "ada{$attempt}@example.com"]))
                ->assertRedirect('/contact');
        }

        $this->post('/contact', $this->validSubmission(['email' => 'ada7@example.com']))
            ->assertStatus(429);

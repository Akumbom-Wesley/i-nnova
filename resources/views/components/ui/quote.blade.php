...
<figure {{ $attributes->class([
    ...
    'border-content/10 bg-paper hover:border-accent/40 hover:shadow-lg hover:shadow-shade/5' => ! $isDark,
]) }}>
    ...
    <blockquote class="mt-6 flex-1 text-lg leading-relaxed {{ $isDark ? 'text-white/90' : 'text-content' }}">
        {{ $quote ?? $slot }}
    </blockquote>

    <figcaption class="mt-8 flex items-center gap-4 border-t pt-6 {{ $isDark ? 'border-white/15' : 'border-content/10' }}">
        @if ($photo)
            <img src="{{ $photo }}" alt="" width="48" height="48" loading="lazy" decoding="async" class="h-12 w-12 shrink-0 rounded-full object-cover">
        @endif

        <div class="min-w-0">
            <p class="truncate font-semibold {{ $isDark ? 'text-white' : 'text-content' }}">{{ $name }}</p>
